...
export function useDeviceSalesReport(from?: string, to?: string) {
  return useQuery({ queryKey: [...KEYS.deviceSales, from, to],   queryFn: () => reportsService.getDeviceSalesSummary(from, to), ...OPTS })
}
export function useStockValueReport() {
  return useQuery({ queryKey: KEYS.stockValue,    queryFn: reportsService.getStockValue,         ...OPTS })
}
export function useSupplierPurchasesReport(from?: string, to?: string) {
  return useQuery({ queryKey: [...KEYS.supplierPurch, from, to], queryFn: () => reportsService.getSupplierPurchases(from, to),  ...OPTS })
}
...
export function useTopCustomersReport(from?: string, to?: string) {
  return useQuery({ queryKey: [...KEYS.topCustomers, from, to],  queryFn: () => reportsService.getTopCustomers(from, to),       ...OPTS })
}
