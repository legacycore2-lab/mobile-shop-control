...
import {
  ...
  User, Lock, Phone, Trash2, AlertTriangle,
} from 'lucide-react'
...
export function SettingsPage() {
  ...
  async function handleFactoryReset() {
    if (resetConfirm !== 'احذف كل البيانات') return setResetError('اكتب جملة التأكيد بالضبط')
    ...
    try {
      ...
      await supabase.from('sale_invoice_devices').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('sale_invoice_products').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('sale_invoices').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('purchase_invoice_devices').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('purchase_invoice_products').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('purchase_invoices').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('mobile_devices').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('products').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('mobile_models').delete().neq('id', '00000000-0000-0000-0000-000000000000')
      await supabase.from('mobile_brands').delete().neq('id', '00000000-0000-0000-0000-000000000000')
    ...
    <>
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        {tab === 'profile' && (
          <div className="space-y-5">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input value={fullName} onChange={e => setFullName(e.target.value)}
                      placeholder="الاسم الكامل" required className={cn(inputCls, 'pr-9')} />
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input value={phone} onChange={e => setPhone(e.target.value)}
                      placeholder="01XXXXXXXXX" dir="ltr" className={cn(inputCls, 'pr-9 text-right')} />
                  </div>
                </div>
                {saveMsg && (
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" value={newPass} onChange={e => setNewPass(e.target.value)}
                      placeholder="6 أحرف على الأقل" minLength={6} className={cn(inputCls, 'pr-9')} />
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" value={newPass2} onChange={e => setNewPass2(e.target.value)}
                      ...
                      className={cn(inputCls, 'pr-9', newPass2 && newPass !== newPass2 && 'border-red-400')} />
                  </div>
                  {newPass2 && newPass !== newPass2 && <p className="text-xs text-red-500">كلمتا المرور غير متطابقتين</p>}
                </div>
                {passMsg && (
                  <div className={cn('flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm border',
            ...
            {/* ── Factory Reset ── */}
            <Section title="إعادة الضبط" sub="مسح كل بيانات النظام بشكل نهائي">
              <div className="rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/20 p-4 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-red-700 dark:text-red-400">تحذير — لا يمكن التراجع</p>
                    <p className="text-xs text-red-600 dark:text-red-500 mt-0.5 leading-relaxed">
                      سيتم حذف جميع الفواتير والأجهزة والمنتجات والموردين والعملاء والمدفوعات نهائياً.
                      بيانات المستخدمين لن تُحذف.
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => { setShowReset(true); setResetStep(1); setResetPassword(''); setResetConfirm(''); setResetError(''); setResetDone(false) }}
                  className="w-full h-10 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-bold transition-colors flex items-center justify-center gap-2"
                >
                  <Trash2 size={15} /> إعادة الضبط إلى الإعدادات الافتراضية
                </button>
              </div>
            </Section>
          </div>
          ...
          <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => !isDark && toggle()}
                  className={cn('flex flex-col items-center gap-3 p-5 rounded-xl border-2 transition-all',
                    !isDark ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700')}>
            ...
            <Section title="اللغة والمنطقة" sub="إعدادات العرض">
              <div className="space-y-0">
                ...
                <StatRow label="تنسيق الأرقام" value="أرقام إنجليزية" />
      ...
      {showReset && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm shadow-2xl" dir="rtl">
            ...
            {resetDone ? (
              <div className="p-8 text-center space-y-4">
                ...
                <p className="text-base font-bold text-gray-900 dark:text-white">تم مسح جميع البيانات</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">النظام جاهز للاستخدام من جديد</p>
                <button onClick={() => { setShowReset(false); window.location.reload() }}
                  className="w-full h-10 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors">
                  إعادة تحميل النظام
                </button>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-bold text-gray-900 dark:text-white">إعادة الضبط النهائية</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500">خطوة {resetStep} من 2</p>
                    </div>
                  </div>
                  <button onClick={() => setShowReset(false)} disabled={resetLoading}
                    className="w-7 h-7 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-colors disabled:opacity-50 text-lg leading-none">
                    ×
                  </button>
                </div>
                ...
                <div className="p-5 space-y-4">
                  {resetStep === 1 && (
                    <>
                      <p className="text-sm text-gray-600 dark:text-gray-400">أدخل كلمة مرور المدير للمتابعة:</p>
                      <input
                        type="password" value={resetPassword}
                        onChange={e => setResetPassword(e.target.value)}
                        placeholder="كلمة المرور" autoFocus
                        className="w-full h-11 border border-gray-200 dark:border-gray-700 rounded-xl px-4 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/10 transition-all"
                      />
                      {resetError && <p className="text-xs text-red-500 text-center font-medium">{resetError}</p>}
                      <div className="flex gap-2">
                        <button onClick={() => setShowReset(false)}
                          className="flex-1 h-10 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          إلغاء
                        </button>
                        <button
                          onClick={async () => {
                            if (!resetPassword) return setResetError('أدخل كلمة المرور')
                            const email = session?.user?.email
                            if (!email) return setResetError('خطأ في التعرف على المستخدم')
                            setResetLoading(true); setResetError('')
                            const { error } = await supabase.auth.signInWithPassword({ email, password: resetPassword })
                            setResetLoading(false)
                            if (error) return setResetError('كلمة المرور غير صحيحة')
                            setResetStep(2); setResetError('')
                          }}
                  ...
                  {resetStep === 2 && (
                    <>
                      <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-xl p-3">
                        <p className="text-xs font-bold text-red-700 dark:text-red-400 mb-1">⚠️ لا يمكن التراجع عن هذه العملية</p>
                        <p className="text-xs text-red-600 dark:text-red-500">سيتم حذف كل الفواتير والأجهزة والمنتجات والموردين والعملاء والمدفوعات</p>
                      </div>
                      <div className="space-y-1.5">
                        <p className="text-xs text-gray-600 dark:text-gray-400 font-medium">
                          اكتب للتأكيد: <span className="font-bold text-red-600">احذف كل البيانات</span>
                        </p>
                        <input
                          type="text" value={resetConfirm}
                          onChange={e => setResetConfirm(e.target.value)}
                          placeholder="احذف كل البيانات" autoFocus
                          className="w-full h-11 border border-red-300 dark:border-red-800 rounded-xl px-4 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-red-300 dark:placeholder:text-red-800 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/10 transition-all"
                        />
                      </div>
                      {resetError && <p className="text-xs text-red-500 text-center font-medium">{resetError}</p>}
                      <div className="flex gap-2">
                        <button onClick={() => { setResetStep(1); setResetError('') }}
                          className="flex-1 h-10 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          رجوع
                        </button>
                        <button
                          onClick={() => void handleFactoryReset()}
                          disabled={resetLoading || resetConfirm !== 'احذف كل البيانات'}
                          className="flex-1 h-10 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                          {resetLoading
                            ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                            : <Trash2 size={13} />}
