  ...
  const [form, setForm] = useState<FormState>(
    customer
      ? { name: customer.name, phone: customer.phone ?? '', address: customer.address ?? '',
          opening_balance: String(customer.opening_balance), notes: customer.notes ?? '',
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {customer ? `تعديل بيانات ${customer.name}` : 'أدخل بيانات العميل الجديد'}
            </p>
          </div>
          ...
          <div className="px-6 py-5 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input type="number" min="0" value={form.opening_balance}
                  onChange={e => set('opening_balance', e.target.value)} placeholder="0"
                  className="h-10 border border-gray-200 dark:border-gray-700 rounded-lg px-3 text-sm bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all" />
              </div>
            </div>
            ...
            {customer && (
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300">عميل نشط</span>
              </label>
            )}
...
export function CustomersPage() {
  ...
  const STATS_CONFIG = [
    { label:'إجمالي العملاء',  value: stats?.total    ?? 0,   icon: Users,      colorClass:'text-blue-600 dark:text-blue-400',   bgClass:'bg-blue-50 dark:bg-blue-900/20'    },
    { label:'عملاء نشطون',     value: stats?.active   ?? 0,   icon: UserCheck,  colorClass:'text-green-600 dark:text-green-400',  bgClass:'bg-green-50 dark:bg-green-900/20'  },
    ...
    { label:'غير نشطين',       value: stats?.inactive ?? 0,   icon: UserX,      colorClass:'text-amber-600 dark:text-amber-400',  bgClass:'bg-amber-50 dark:bg-amber-900/20'  },
  ]
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">العملاء</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">إدارة بيانات وحسابات العملاء</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => exportToCsv('customers', CUSTOMER_EXPORT_HEADERS, filtered)} className="h-9 px-4 text-sm font-medium rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors flex items-center gap-2">
        ...
        <div className="flex gap-1.5">
          {(['all', 'active', 'inactive'] as const).map((v) => (
            <button key={v} onClick={() => { setFilter(v); setPage(1) }}
              className={cn('h-8 px-3 text-xs font-medium rounded-lg border transition-colors',
                filter === v
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                {['#','اسم العميل','التليفون','العنوان','الرصيد','المشتريات','الحالة','إجراءات'].map((h, i) => (
                  <th key={h} className={cn('px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap text-right', i >= 4 && 'text-center')}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <tr key={c.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-xs text-gray-700 dark:text-gray-300">{c.phone ?? '—'}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <MapPin size={12} className="text-gray-400 dark:text-gray-600 flex-shrink-0" />
                      <span className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-36">{c.address ?? '—'}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={cn('font-semibold text-sm', (c.opening_balance ?? 0) > 0 ? 'text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-500')}>
                      {(c.opening_balance ?? 0).toLocaleString('ar-EG')} ج
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1 text-gray-400 dark:text-gray-600">
                      <ShoppingBag size={12} />
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border',
                      ...
                      <span className={cn('w-1.5 h-1.5 rounded-full', c.is_active ? 'bg-green-500' : 'bg-red-500')} />
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5 justify-center">
                      ...
                      <button title="تعديل" onClick={() => openEdit(c)}
                      ...
                      <button title="حذف" onClick={() => handleDelete(c.id)}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-gray-800">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <span className="font-semibold text-gray-900 dark:text-white">{customers.length}</span> عميل
          </p>
          {totalPages > 1 && (
