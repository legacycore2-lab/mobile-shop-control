...
<div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <KpiCard label="إجمالي الأجهزة"      value={customers.reduce((s: number, r: any) => s + Number(r.device_count),0)}                 icon={Smartphone} color="teal"  />
            <KpiCard label="إجمالي المبيعات"     value={`${fmt(customers.reduce((s: number, r: any) => s + Number(r.total_spent),0))} ج`}     icon={DollarSign} color="green" />
            <KpiCard label="متوسط إنفاق العميل"  value={`${fmt(customers.length?customers.reduce((s: number, r: any) => s + Number(r.total_spent),0)/customers.length:0)} ج`} icon={TrendingUp} color="amber" />
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  ...
                  : customers.map((r: any, i: number) =>{
                    const total = customers.reduce((s: number, x: any) =>s+x.total_spent,0)
                    return (
