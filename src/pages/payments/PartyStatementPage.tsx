...
function SimplePayModal({
  invoiceId, invoiceNumber, partyId, partyName, partyType, paymentType, remaining, onClose
}: {
  invoiceId: string; invoiceNumber: string; partyId: string; partyName: string
  partyType: 'supplier' | 'customer'; paymentType: 'purchase' | 'sale'
  remaining: number; onClose: () => void
}) {
  ...
  async function handlePay() {
    ...
    try {
      await createPayment.mutateAsync({
        payment_type: paymentType, invoice_id: invoiceId, invoice_number: invoiceNumber,
        party_type: partyType, party_id: partyId,
        amount: amt, payment_method: method, payment_date: date,
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4"
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-xs shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <div>
            <p className="text-sm font-bold text-gray-900 dark:text-white">تسجيل دفعة</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{invoiceNumber} — المتبقي: {fmt(remaining)} ج</p>
          </div>
          <button onClick={onClose}
            className="w-7 h-7 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:text-gray-600">
        ...
        <div className="p-5 space-y-3">
          ...
          <input type="number" min="0.01" step="0.01" value={amount}
            onChange={e => setAmount(e.target.value)} placeholder="المبلغ (ج)"
            className={inp + ' text-xl font-bold text-center'} autoFocus />
        ...
        <div className="px-5 pb-5">
          <button onClick={() => void handlePay()} disabled={saving}
            ...
            <DollarSign size={15} /> تسجيل الدفعة
          </button>
        </div>
      </div>
    </div>
