...
import {
  ...
  Calendar, CreditCard, Receipt, ChevronDown, X,
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="0.00"
              className={cn(inp, 'h-14 text-2xl font-bold text-center')}
              autoFocus
            />
          </div>
            ...
            {showCat && (
              <div className="flex gap-2 mb-2">
                ...
                <button onClick={() => void handleAddCategory()}
                  className="h-10 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition-colors">
                  إضافة
                </button>
              </div>
            )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: `${fmt(stats?.thisMonth ?? 0)} ج`,
            icon: Calendar,
            color: 'amber' as const,
            sub: stats?.lastMonth ? `الشهر الماضي: ${fmt(stats.lastMonth)} ج` : undefined,
          },
          {
            label: 'أكبر فئة إنفاق',
            value: stats?.byCategory[0]?.category_name ?? '—',
            icon: Receipt,
            color: 'purple' as const,
            sub: stats?.byCategory[0] ? `${fmt(stats.byCategory[0].total)} ج` : undefined,
          },
          {
            label: 'إجمالي المعروض',
            value: `${fmt(filteredTotal)} ج`,
            icon: Wallet,
            color: 'blue' as const,
            sub: `${filtered.length} من ${expenses.length}`,
          },
          ...
          const colors = {
            ...
            purple: { bg: 'bg-purple-50 dark:bg-purple-900/20', icon: 'text-purple-500', border: 'border-purple-100 dark:border-purple-900' },
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {isLoading ? (
                ...
                <tr key={exp.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300 max-w-48 truncate">
                    {exp.description ?? '—'}
                  </td>
            ...
            {filtered.length > 0 && (
              <tfoot>
                <tr className="bg-red-50 dark:bg-red-900/10 border-t-2 border-red-200 dark:border-red-800">
                  ...
                  <td className="px-4 py-3 text-sm font-bold text-red-600 dark:text-red-400 whitespace-nowrap">
                    {fmt(filteredTotal)} ج
                  </td>
                  <td />
                </tr>
