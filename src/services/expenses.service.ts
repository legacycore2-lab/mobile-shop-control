...
export const expensesService = {
  ...
  update: async (id: string, form: Partial<ExpenseFormData>, userId?: string): Promise<void> => {
    ...
    if (userId) void logAction({ userId, action: 'update', table: 'expenses', recordId: id, description: 'تعديل مصروف', newData: payload as Record<string, unknown> })
  },

  remove: async (id: string, userId?: string): Promise<void> => {
    if (userId) void logAction({ userId, action: 'delete', table: 'expenses', recordId: id, description: 'حذف مصروف' })
